...
- **插件加载（必读）**：**`~/.hydro/addon.json`** 中插件条目建议使用 **`/root/hydrooj-plugin-api` 绝对路径**；插件根目录**必须**存在 **`index.ts`（或 `index.js`）** 导出 **`apply`**（Hydro **`entry/common.ts`** 不读 **`package.json` 的 `main`**）。否则插件**静默未加载**。完整核对表见 **`docs/SYNC-CADDY-ROUTING.md` §6**。
- **同步协议 P1（WebSocket）**：**`WS /edu-sync-conn`**（**`SyncConnectionHandler`**），监听 **`record/change`**、**`domain/delete-cache`** 等事件并对当前用户 **`$inc userDataVersion`** 后推送 **`sync/version`**。部署与浏览器自检见 **`docs/SYNC-CADDY-ROUTING.md` §7**。
- **前端**（在 `hydroforwindows` 仓库）：**`syncApi.js`**、**`syncConnection.js`**、**`syncDispatcher.js`**、**`AuthContext`**；登录后 **WS + 轮询回退**；登出清理。
